...
name_uni ='testing-Sw_max-average-rp-mld'
...
mld0 = mld.copy()
if mld_analysis_plot or mld_transient:

    mld = mld*np.ones((nt-n_skip, num_cases))
    mass_ratio = np.zeros((nt-n_skip, num_cases))
    bw_mld_avg = np.zeros((nt-n_skip, num_cases))
    w_mld_rms = np.zeros((nt-n_skip, num_cases))
    ...
    for i, folder in enumerate(folders):
        ...
        if mld_transient or mld_analysis_plot:
            if it == 0:
                ...
                mld[it - n_skip, i] = -z[i, mld_idx[i]]
                mass_ratio[it - n_skip,  i] = np.sum(S[:, :, mld_idx[i]::])/np.sum(S)
            w_rms = np.sqrt(w2_fluc_avg)
            w_mld_rms[it - n_skip, i] = w_rms[mld_idx[i]]
        ...
        bw_mld_avg[it - n_skip,  i] = bw_fluc_avg[mld_idx[i]]
...
if ND: 
    ...
    if not mld_transient:
        mld = mld0*np.ones((nt-n_skip, num_cases))
    ...
    for i in range(num_cases):
        ...
        bw_mld_avg[:, i] = bw_mld_avg[:, i] / b_scale[i] / vel_scale[i]
    ...
    mld_plot = np.ones((nt-n_skip, num_cases))
    ...
    if not mld_transient:
        mld_plot = mld0*np.ones((nt-n_skip, num_cases))
        mld0 = np.ones((nt-n_skip, num_cases))
    else:
        mld_plot = mld
# plot ranges
ranges = plot_ranges(lz = 96, rho0 = rho0, T0 = T0, dTdz = np.max(dTdz), Sj = np.max(Sj))
