...
def stokes_exp(z):
    ...
    us = amplitude**2* wavenumber* frequency 
...
name = 'plume-contour-function-NBP-'
...
#ranges = plot_ranges(lz = 96, rho0 = rho0, T0 = T0, dTdz = dTdz, Sj = Sj)
#ranges['w'] = [-2e-4, 2e-4]#ranges['w'] = [-1*10**(-2), 1*10**(-2)]
#ranges['v'] = [-2e-4, 2e-4]
#ranges['restress'] = [-3*10**(-5), 3*10**(-5)]
#ranges['B'] = [-1.5*10**(-3), 5*10**(-4)]
#ranges['bw_fluc'] = [-7.0*10**(-8), 7.0*10**(-8)]
#ranges['vel'] = [-3.0e-3, 3.0e-3]
#ranges['vel_rms'] = [0, 0.01]
...
for it in np.arange(nt):
    ...
    if (vert_slice_plot or buoyancy_analysis_plot or xy_plot or turb_stats_plot) and rho_IC_perturb:
        # calculating density 
        rho_perturbed = ((b - b_background)*rho0)/(-g)
        b_fluc = b - b_background
    elif (vert_slice_plot or buoyancy_analysis_plot or xy_plot or turb_stats_plot) and not rho_IC_perturb:
        # calculating density 
    ...
    if turb_stats_plot:
        if salinity:
            ...
            turb_stat_dir = turb_stats(time, it, ranges, output_folder, lx, nx, z, zf, ml, u_avg, v_avg, w_avg, u_rms, v_rms, w_rms, uv_fluc_avg, uw_fluc_avg, vw_fluc_avg, bu_fluc_avg, bv_fluc_avg, bw_fluc_avg, b_rms, rho_avg, plume_info)
        else:
            turb_stat_dir = turb_stats(time, it, ranges, output_folder, lx, nx, z, zf, ml, u_avg, v_avg, w_avg, u_rms, v_rms, w_rms, uv_fluc_avg, uw_fluc_avg, vw_fluc_avg, bu_fluc_avg, bv_fluc_avg, bw_fluc_avg, b_rms, rho_avg)
    if video_3d_flag:
