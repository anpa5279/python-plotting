...
for it, t in enumerate(t_save):
    ...
    for i, folder in enumerate(folders):
        ...
        uv_fluc, uv_fluc_avg_temp = reynolds_stress(u, v, u_avg[i], v_avg[i])
        uw_fluc, uw_fluc_avg = reynolds_stress(u, w, u_avg[i], w_avg[i])
        vw_fluc, vw_fluc_avg = reynolds_stress(v, w, v_avg[i], w_avg[i])

        b2_fluc, b2_fluc_avg = reynolds_stress(b, b, b_avg[i], b_avg[i])
        bu_fluc, bu_fluc_avg_temp = reynolds_stress(b, u, b_avg[i], u_avg[i])
        bv_fluc, bv_fluc_avg_temp = reynolds_stress(b, v, b_avg[i], v_avg[i])
        bw_fluc, bw_fluc_avg_temp = reynolds_stress(b, w, b_avg[i], w_avg[i])
