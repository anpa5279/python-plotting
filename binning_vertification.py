...
axes[1].plot(r_opt, np.poly1d(coef)(r_opt), color=color_opt[0], label=rf'a={coef[0]:.2e}(dr/$\sigma$) + {coef[1]:.2f}', linestyle=line_opt[1], linewidth = 0.6)
coef = scipy.optimize.curve_fit(quadratic_fit, dr_nd, int_area/area_true)[0]
axes[1].plot(r_opt, quadratic_fit(r_opt, *coef), color=color_opt[0], label=rf'a={coef[0]:.2e}(dr/$\sigma$)$^2$ + {coef[1]:.2f}', linestyle=line_opt[2], linewidth = 0.6)

for i, dr in enumerate(dr_nd):
    axes[1].scatter(dr, int_area[i]/area_true, color=color_opt[i+1], marker='x')
...
axes[2].plot(r_opt, log_10_fit(r_opt, *coef), color=color_opt[0], label=rf'error=(dr/$\sigma)^{{{coef[0]:.3f}}}$ 10$^{{{coef[1]:.3f}}}$', linestyle=line_opt[2], linewidth = 0.6)
