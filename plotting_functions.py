...
def plot_binning(S_rz, T_fluc_rz, T_rz, hor_vel_rz, w_rz, r, z, time, output_folder, min_S = 10**(-6)):
    ...
    umax = np.max(np.abs(hor_vel_rz)) * frac
    u_range = (-umax, umax)
    ...
    for it, t in enumerate(time):
        fig, ax = plt.subplots(1, 5, figsize=(16, 3.5), sharey = True, sharex = True, constrained_layout=True, dpi = 300)
        ax = ax.ravel()
        td = t / 3600 / 24
        ...
        ax[1].set_aspect('equal')
        cbar = fig.colorbar(im, ax = ax[1], label=r"$^\circ$C", anchor = (0.5, -0.05), orientation='horizontal', shrink=0.75)
        ...
        ax[2].set_aspect('equal')
        cbar = fig.colorbar(im, ax = ax[2], label=r"g/kg", anchor = (0.5, -0.05), orientation='horizontal', shrink=0.75)
        ...
        im = ax[4].imshow(w_rz[:, :, it].T, vmin=w_range[0], vmax=w_range[1], extent =[r.min(), r.max(), z.min(), z.max()], interpolation ='none', origin ='lower', cmap='RdBu_r')
