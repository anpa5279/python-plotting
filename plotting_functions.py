...
def xy_plane_slices(time, it, ranges, fig_folder, lx, X, Y, u, v, w, b, b_fluc, Pdynamic, rho, rho_perturbed, idx_loc, plane, T = np.array([]), S = np.array([])):
    ...
    ax1.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
    ax1.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
    ax1.set_aspect('equal')
    ...
    ax2.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
    ax2.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
    ax2.set_aspect('equal')
    ...
    if np.size(S)==0:
        ...
        ax3.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
        ax3.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
        ax3.set_aspect('equal')
    ...
    if np.size(T)==0:
        ...
        #ax4.set_xlabel("[m]")
        ax4.set_title("Buoyancy Perturbations")
        ax4.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
        ax4.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
        ax4.set_aspect('equal')
    ...
    ax5.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
    ax5.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
    ax5.set_aspect('equal')
    ...
    ax6.set_xlabel("[m]")
    ax6.set_title("u")
    ax6.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
    ax6.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
    ax6.set_aspect('equal')
    ...
    ax7.set_xlabel("[m]")
    ax7.set_title("v")
    ax7.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
    ax7.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
    ax7.set_aspect('equal')
    ...
    #ax8.set_ylabel("[m]")
    ax8.set_title("w")
    ax8.set_ylim(np.min(Y[:, :, idx_loc]), np.max(Y[:, :, idx_loc]))
    ax8.set_xlim(np.min(X[:, :, idx_loc]), np.max(X[:, :, idx_loc]))
    ax8.set_aspect('equal')
