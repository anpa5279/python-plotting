...
def plot_rig_exponents(color_opt, time, it, fig_folder, w_rms, b_center, bw, rp, T, S, z_nd, zf_nd, Ri_g, case_names):
    num_cases = len(case_names)
    outdir = os.path.join(fig_folder, "ND strat influence")
    os.makedirs(outdir, exist_ok=True)
    exponents = [-0.5, -1/3, -0.25, 0.0, 0.25, 1/3, 0.5]
    scale = np.ones(7) 
    scale[-1] = 0.02
    gridspec_kw={'height_ratios': scale}
    fig, axes = plt.subplots(7, len(exponents), figsize=(30, 31), sharey=True, gridspec_kw = gridspec_kw)
    for a in axes[-1, :]:
        a.remove()
    ...
    for ax, exp in zip(axes[0, :], exponents):
        ...
        ax.set_title(f'Ri_g^{exp:.2f}')
        ax.set_xlabel(r'$w_{{rms}}/\sqrt{\text{g l}_{j}} \cdot Ri_g^{{{exp}}}$')
    #axes[0, 0].legend()
    ...
    for ax, exp in zip(axes[1, :], exponents):
        ...
        ax.ticklabel_format(axis='x', style='sci', scilimits=(-3,2), useMathText=True)
        ax.set_title(f'Ri_g^{exp:.2f}')
        ax.set_xlabel(r'$b_{centerline}/g \cdot Ri_g^{{{exp}}}$')
    #axes[1, 0].legend()
    ...
    for ax, exp in zip(axes[2, :], exponents):
        ...
        ax.ticklabel_format(axis='x', style='sci', scilimits=(-3,2), useMathText=True)
        ax.set_title(f'Ri_g^{exp:.2f}')
        ax.set_xlabel(r"$(\langle b'w'\rangle_{xy}/\sqrt{\text{g}^3 \text{l}_{j}})\cdot Ri_g^{{{exp}}}$")
    #axes[2, 0].legend()
    ...
    for ax, exp in zip(axes[3, :], exponents):
        ...
        ax.ticklabel_format(axis='x', style='sci', scilimits=(-3,2), useMathText=True)
        ax.set_title(f'Ri_g^{exp:.2f}')
        ax.set_xlabel(r"(r/l$_{j}\cdot Ri_g^{{{exp}}}$")
    #axes[3, 0].legend()
    ...
    for ax, exp in zip(axes[4, :], exponents):
        ...
        ax.ticklabel_format(axis='x', style='sci', scilimits=(-3,2), useMathText=True)
        ax.set_title(f'Ri_g^{exp:.2f}')
        ax.set_xlabel(r"$(\text{T'}_{\text{centerline}}\alpha)\cdot Ri_g^{{{exp}}}$")
    #axes[4, 0].legend()
    ...
    for ax, exp in zip(axes[5, :], exponents):
        ...
        ax.ticklabel_format(axis='x', style='sci', scilimits=(-3,2), useMathText=True)
        ax.set_title(f'Ri_g^{exp:.2f}')
        ax.set_xlabel(r"($\langle$C$\rangle_{\text{xy}} \beta)\cdot Ri_g^{{{exp}}}$")
    #axes[5, 0].legend()

    plt.tight_layout()

    # --- Save Frame ---
    frame_path = os.path.join(outdir, f"{it:04d}.png")
