...
### ---------------------- DENSE PLUME ----------------------------- ###
def plume_temporal_analysis(time, ranges, color_opt, fig_folder, case_names, name, lx, start_neutral, mld, h_neutral, h_max, r_mld, r_neutral, r_hmax, w_mld, w_neutral, w_hmax, b_mld, b_neutral, b_hmax, T_mld, T_neutral, T_hmax, tracer_mld, tracer_neutral, tracer_hmax, tracer_avg, w_rms, ND = False):
    num_cases = len(case_names)
    if num_cases==1:
...
def plume_spatial_analysis(time, it, ranges, color_opt, fig_folder, case_names, name, lx, z, zf, tracer_avg, u_rms, v_rms, w_rms, b_avg, b_center, r_profile, bu_fluc_avg, bv_fluc_avg, bw_fluc_avg, b_rms, T_fluc, tracer_fluc, ND = False):
    num_cases = len(case_names)
    if num_cases==0:
    ...
    for i in range(num_cases):
        if i == 0:
            ax3.plot(b_avg[:, i], z[:, i], color = color_opt[i], label = r"b$_{\text{average}}$", linestyle='solid')
